...
class BusinessRequirementsController < ApplicationController
  before_action :set_business_requirement, only: [:show, :edit, :update, :destroy, :accept, :decline, :send_email, :meeting]
  helper :attachments
  ...
  def index
    ...
    filters.each do |key, value|
      ...
      status_keys = value.filter_map { |v| BusinessRequirement::STATUS_MAP.key(v) }
      if  key == :status
    ...
    @statuses = @business_requirements.filter_map { |br| BusinessRequirement::STATUS_MAP[br.status] }.uniq.sort
  ...
  def export_all
    ...
    if  params[:statuses].present?
      status_keys = params[:statuses].filter_map { |v| BusinessRequirement::STATUS_MAP.key(v) }
  ...
  def accept
    ...
    if @business_requirement && @business_requirement.update(business_requirement_params)
      ...
      mandatory_fields.each do |field|
        ...
        if [:portfolio_category, :requirement_received_from].include?(field)
      ...
      custom_fields.each do |field_name, value|
        ...
        when "Portfolio Category", "Application Name", "Function"
          ...
          custom_values = value.reject(&:blank?).filter_map do |enumeration_name|
  ...
  def create_issues_for_field_names(project, business_requirement, field_names, issues, plan_tracker)
    if  Issue.where(tracker_id: plan_tracker.id, project_id: project.id).count == 0
      ...
      issues.each do |issue|
        ...
        @new_issue.assigned_to_id = (project_manager_user_id.presence || User.current.id)
...
content_type: attachment.content_type, digest: attachment.digest, downloads: 0, author_id: User.current.id, description: attachment.description, disk_directory: attachment.disk_directory)
    end
  rescue => e
  end
